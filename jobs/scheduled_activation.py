...
async def _try_send_notification(
    ...
    keyboard = start_notification_keyboard(
        ...
        group_url=plan.telegram_group_url if plan else None,
    )
...
async def process_reminders(
    ...
    for p in cache.all_participants():
        ...
        # У нагадування за день додаємо групу потоку, якщо вона задана в тарифі.
        keyboard = reminder_keyboard(
            chat_url=plan.chat_url,
            curator_url=plan.curator_url,
            group_url=plan.telegram_group_url if hours_to_send == 24 else None,
        )
